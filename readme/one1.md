...
<h4>1-2. 패러다임의 불일치</h4><hr>
    데이터를 저장하기 위해 데이터베이스를 사용하지만 데이터베이스는 추상화, 상속, 다형성 같은 개념이 없다.<br/>
...
JPA는 자바 진영의 ORM(Object-relational Mapping) 기술 표준이다.<br>
ORM이란 객체와 관계형 데이터베이스를 맵핑하는것이다.<br>
ORM프레임워크는 개발자 대신 1-2에서 설명한 패러다임의 불일치 문제를 해결해준다.<br>
ORM프레임워크는 개발자가 어떻게 맵핑할지만 ORM 프레임워크에게 알려주면 객체지향 어플리케이션 개발에 집중할 수 있다.<br>
자바에서는 하이버네이트(hibernate)가 대표적인 ORM 프레임워크이다.<br>
<br>
JPA를 사용해야하는 이유를 하나씩 살펴보자.<br>
<br>
<br>
1.생산성<br>
JPA를 사용하면 CRUD용 SQL를 개발자가 작성하지 않아도 되므로, 객체 설계중심의 개발을 할 수 있다.<br><br>
2.유지보수<br>
모델에 필드를 추가하면 해당되는 SQL을 추가로 설정하고 맵핑하기위한 JDBC API코드를 모두 변경해야하는데 이런 번거로움을 없애준다.<br><br>
3.패러다임의 불일치 해결<br>
1-2에서 살펴본 상속, 연관관계, 객체그래프탐색, 비교하기등의 패러다임 문제를 해결해 줄 수있다.<br><br>
4.성능<br>
<pre><code>String memberId = "helloId";
Member member1 = jpa.find(memberId);
Member member2 = jpa.find(memberId);</code></pre>
JPA는 위와같은 코드가 트랜잭션 안에서 실행된다고 쳤을 때 기존 JDBC API는 2번 SELECT를 할 코드를 한 번만 실행하고 두번째는 조회한 회원 객체를 재사용한다.<br><br>
5.데이터 접근 추상화 벤더 독립성<br>
데이터베이스마다 같은 기능도 사용법이 다른 경우가 많은데 JPA는 추상화를 통해 특정 기술에 종속되지않고 개발할 수 있게 도와준다.<br>
<h4>1-4. 정리</h4><hr>
지금까지 SQL을 직접 다룰 때 발생하는 다양한 문제와 패러다임 불일치 문제를 알아봤다<br>
그리고 JPA의 장점을 정리해 볼 수 있었다.<br>
다음장부터는 간단한 CRUD JPA 어플리케이션을 만들어보자!